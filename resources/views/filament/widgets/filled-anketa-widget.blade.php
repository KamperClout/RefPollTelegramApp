<x-filament::widget>
    <x-filament::card>
        <div>
            <div class="text-sm text-gray-400">Заполнено за 7 дней</div>
            <div class="text-3xl font-bold">{{ $this->getData()['current'] }}</div>
            @php
                $current = $this->getData()['current'];
                $previous = $this->getData()['previous'] ?? 0;
        ...
        <svg viewBox="0 0 100 24" width="100%" height="28" fill="none" preserveAspectRatio="none" style="margin-top: 12px;">
            ...
            @else
                <path d="M0 18 Q 10 24 20 12 Q 30 0 40 12 Q 50 24 60 12 Q 70 0 80 12 Q 90 24 100 18 L100 24 L0 24 Z" style="{{ $fillColor }}" />
                <path d="M0 18 Q 10 24 20 12 Q 30 0 40 12 Q 50 24 60 12 Q 70 0 80 12 Q 90 24 100 18" stroke-width="2.5" style="{{ $lineColor }}" fill="none"/>
            @endif
        </svg>
    </x-filament::card>
</x-filament::widget>
